...
class MLBProjection:
    ...
    def total_probability(self, line: float, direction: Literal["over", "under"]) -> float:
        # Honest independent projection. Extreme edges are REVIEW'd downstream —
        # do not silently shrink toward 50% to hide calibration problems.
        over = 1 - _normal_cdf(line, self.expected_total_runs, 3.35)
    ...
    def spread_probability(self, side: Literal["home", "away"], line: float) -> float:
        ...
        cover_probability = 1 - _normal_cdf(-line, side_margin, 3.45)
...
def project_mlb_game(
    *,
    ...
    if away_bullpen.get("heavy_usage"):
        expected_total += 0.25
    expected_total = _clamp(expected_total, 5.0, 12.5)
